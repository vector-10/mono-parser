...
export default function ReviewWorkspacePage() {
  ...
  const amount = isCounter
    ? d?.counter_offer?.counter_amount
    : d?.approval_details?.approved_amount;
  const tenor = isCounter
    ? d?.counter_offer?.counter_tenor
    : d?.approval_details?.approved_tenor;
  ...
  return (
    <div className="-mx-6 -my-8 h-[calc(100vh-4rem)] flex flex-col">
      <div className="flex items-center gap-4 px-6 py-4 bg-white border-b border-gray-100 shrink-0">
        ...
        <span className="px-2 py-0.5 rounded-full bg-[#0055ba]/10 text-[#0055ba] text-xs font-medium">
          Manual Review
        </span>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-105 border-r border-gray-100 overflow-y-auto custom-scrollbar bg-white shrink-0">
          <div className="px-6 py-6 space-y-6">
            ...
            {app.score && band && (
              <div className="border border-gray-100 rounded-xl p-5">
                ...
                <div className="flex items-end gap-3 mb-4">
                  ...
                  <p className={`text-sm font-medium mb-1 ${band.color}`}>
                    {band.label}
                  </p>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-1.5 mb-3">
                  <div
                    className={`h-1.5 rounded-full ${band.bar} transition-all`}
            ...
            {d?.explainability?.strengths?.length ||
            d?.explainability?.weaknesses?.length ? (
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {terms?.monthly_payment
                        ? formatNaira(terms.monthly_payment)
                        : "—"}
                    </p>
                  </div>
                </div>
            ...
            <div className="flex gap-3 pt-2 pb-4">
              <button
                disabled
                className="flex-1 flex items-center justify-center gap-2 bg-[#59a927] text-white px-4 py-3 rounded-xl text-sm font-semibold opacity-50 cursor-not-allowed"
              >
                <RiCheckLine className="w-4 h-4" /> Approve
              </button>
              <button
                disabled
                className="flex-1 flex items-center justify-center gap-2 border border-red-200 text-red-500 px-4 py-3 rounded-xl text-sm font-semibold opacity-50 cursor-not-allowed"
              >
                <RiCloseLine className="w-4 h-4" /> Decline
              </button>
            </div>
          </div>
        </div>
