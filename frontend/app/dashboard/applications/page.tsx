...
import { RiSearchLine, RiCloseLine, RiArrowRightSLine, RiFileList3Line } from "react-icons/ri";
...
type DecisionData = {
  ...
  explainability?: { primary_reason?: string; strengths?: string[]; weaknesses?: string[] };
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          {app.score && (
            <div className="border border-gray-100 rounded-xl p-5">
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-4">Credit Assessment</p>
              <div className="flex items-end gap-4 mb-4">
                <p className="font-mono text-4xl font-medium text-[#0055ba]">{app.score}</p>
                <div className="mb-1">
                  <p className="text-xs text-gray-400">{scoreBand(app.score)}</p>
                  {decisionKey && (
                    <span className={`text-sm font-semibold ${DECISION_STYLE[decisionKey] ?? "text-gray-600"}`}>
          ...
          {(d?.approval_details || d?.counter_offer) && (
            <div className="bg-[#59a927]/5 border border-[#59a927]/20 rounded-xl p-5">
              ...
              {(() => {
                ...
                const tenor = d.counter_offer ? terms?.counter_tenor : terms?.approved_tenor;
                return (
          ...
          {d?.risk_factors && d.risk_factors.length > 0 && (
            <div>
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">Risk Factors</p>
              <ul className="space-y-2">
          ...
          <div className="pt-2 border-t border-gray-50">
            ...
            {app.processedAt && (
              <p className="text-xs text-gray-400">Processed {formatDate(app.processedAt)}</p>
            )}
          </div>
        </div>
      </aside>
...
export default function ApplicationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return applications.filter((a) => {
      const name = a.applicant ? `${a.applicant.firstName} ${a.applicant.lastName}`.toLowerCase() : "";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">All loan applications submitted through your API.</p>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {STATUSES.map(({ key, label }) => (
          ...
          >
            {label}
            {counts[key] > 0 && (
              <span className={`text-xs ${activeStatus === key ? "opacity-70" : "text-gray-400"}`}>
                {counts[key]}
              </span>
            )}
          </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50">
                {["Applicant", "Amount", "Tenor", "Score", "Decision", "Status", "Date", ""].map((col) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((app) => {
                const d = app.decision as DecisionData | null;
                const decisionKey = d?.decision ?? "";
                const status = STATUS_STYLE[app.status] ?? "bg-gray-100 text-gray-500";
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-3.5 text-gray-500 text-xs">{app.tenor}mo</td>
                    ...
                    <td className={`px-6 py-3.5 text-xs font-medium ${DECISION_STYLE[decisionKey] ?? "text-gray-300"}`}>
