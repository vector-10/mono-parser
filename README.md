...
│          Dashboard the fintech company logs into          │
│       View applications · See decisions · Manage keys     │
└──────────────────────────┬───────────────────────────────┘
                           │ REST + WebSocket
                           ▼
┌──────────────────────────────────────────────────────────┐
...
│                                                          │
│  ┌────────────┐  ┌─────────────┐  ┌──────────────────┐  │
│  │  REST API  │  │  WebSocket  │  │  BullMQ Workers  │  │
│  │ (auth,apps,│  │(live updates│  │ (process jobs in │  │
│  │  webhooks) │  │ to dashboard│  │  background)     │  │
│  └────────────┘  └─────────────┘  └──────────────────┘  │
└───┬─────────┬──────────┬──────────────────┬──────────────┘
    │         │          │                  │
    ▼         ▼          ▼                  ▼
┌────────┐ ┌──────┐ ┌─────────────┐  ┌──────────────┐
│FastAPI │ │Prisma│ │   BullMQ    │  │ Mono Connect │
│Brain   │ │  ↓   │ │   Queue     │  │ API          │
│:8000   │ │Post- │ │    ↓        │  │ (external)   │
│        │ │greSQL│ │  Redis      │  └──────┬───────┘
│Scores  │ │:5432 │ │  :6379      │         │
│the loan│ │      │ │             │         │ Sends webhooks
│        │ │Stores│ │Holds pending│         │ back when user
│Returns │ │every-│ │jobs until a │         │ links account
│decision│ │thing │ │worker picks │         │
└────────┘ └──────┘ │them up      │  ◄──────┘
                    │             │
                    │  Workers send:
                    │  ┌──────────────────────┐
